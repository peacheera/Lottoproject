...
public class ObjectTest {
	public static void main(String[] args) {
		...
		//  Clonable 인터페이스를 수현해야 클론이 가능
	
		try {
		Person origin = new Person("홍길동", 20);
		Person clone = (Person)origin.clone(); 		// 객체복제
		System.out.println(origin.hashCode()); 	// 해쉬코드 값
		...
		System.out.println(origin.toString()); // Person 에서 오버라이딩 한 toString 호출 객체의 문자열 표현
		System.out.println(origin.name + " " + clone.age);
